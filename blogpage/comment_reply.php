<?php 
	...
	$post_id = $_GET['topic'];
	// $post_query_result = mysqli_query($db, "SELECT * FROM posts WHERE id=$post_id");
	// $post = mysqli_fetch_assoc($post_query_result);

	// Get all comments from database
	$comments_query_result = mysqli_query($db, "SELECT * FROM comments WHERE post_id=$post_id ORDER BY created_at DESC") or die(mysqli_error($db));
...
if (isset($_POST['comment_posted'])) {
	...
	$sql = "INSERT INTO comments (post_id, user_id, body, created_at, updated_at) VALUES (6 , " . $user_id . ", '$comment_text', now(), null)";
